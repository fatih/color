...
func TestFormatPrint(t *testing.T) {
	...
	for _, c := range testColors {
		// FormatPrint
		colored := fmt.Sprintf("\x1b[%dm%-10s\x1b[0m", c.code, c.text)
		ffmt := New(c.code).FormatPrint(c.text)
		line := fmt.Sprintf("%-10s", ffmt)
		scannedLine := fmt.Sprintf("%q", line)
		escapedForm := fmt.Sprintf("%q", colored)
		...
		// FormatPrintln
		colored = fmt.Sprintf("\x1b[%dm%-10s\x1b[0m", c.code, c.text+"\n")
		ffmt = New(c.code).FormatPrintln(c.text)
		line = fmt.Sprintf("%-10s", ffmt)
		scannedLine = fmt.Sprintf("%q", line)
		escapedForm = fmt.Sprintf("%q", colored)
		...
		// FormatPrintf
		colored = fmt.Sprintf("\x1b[%dm%-10s\x1b[0m", c.code, c.text)
		ffmt = New(c.code).FormatPrintf("%s", c.text)
		line = fmt.Sprintf("%-10s", ffmt)
		scannedLine = fmt.Sprintf("%q", line)
		escapedForm = fmt.Sprintf("%q", colored)
...
func TestFormatFunc(t *testing.T) {
	...
	for _, c := range testColors {
		// FormatPrint
		colored := fmt.Sprintf("\x1b[%dm%-10s\x1b[0m", c.code, c.text)
		ffmt := New(c.code).FormatFunc()(c.text)
		line := fmt.Sprintf("%-10s", ffmt)
		scannedLine := fmt.Sprintf("%q", line)
		escapedForm := fmt.Sprintf("%q", colored)
		...
		// FormatPrintln
		colored = fmt.Sprintf("\x1b[%dm%-10s\x1b[0m", c.code, c.text+"\n")
		ffmt = New(c.code).FormatlnFunc()(c.text)
		line = fmt.Sprintf("%-10s", ffmt)
		scannedLine = fmt.Sprintf("%q", line)
		escapedForm = fmt.Sprintf("%q", colored)
		...
		// FormatPrintf
		colored = fmt.Sprintf("\x1b[%dm%-10s\x1b[0m", c.code, c.text)
		ffmt = New(c.code).FormatfFunc()("%s", c.text)
		line = fmt.Sprintf("%-10s", ffmt)
		scannedLine = fmt.Sprintf("%q", line)
		escapedForm = fmt.Sprintf("%q", colored)
